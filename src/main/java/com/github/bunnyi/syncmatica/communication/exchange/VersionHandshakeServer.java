...
public class VersionHandshakeServer extends FeatureExchange {
    ...
    public void handle(final Identifier id, final PacketByteBuf packetBuf) {
        if (id.equals(PacketType.REGISTER_VERSION.identifier)) {
            ...
            if (!context.checkPartnerVersion(partnerVersion)) {
                LogManager.info("Denying syncmatica join due to outdated client with local version {} and client version {}", SyncmaticaPlugin.VERSION, partnerVersion);
                close(false);
                return;
            }
    ...
    public void onFeatureSetReceive() {
        LogManager.info("Syncmatica client joining with local version {} and client version {}", SyncmaticaPlugin.VERSION, partnerVersion);
        PacketByteBuf newBuf = new PacketByteBuf();
        Collection<ServerPlacement> collection = context.syncmaticManager.getAll();
    ...
    public void init() {
        final PacketByteBuf newBuf = new PacketByteBuf();
