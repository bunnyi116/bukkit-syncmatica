...
public class ServerPlacement {
    ...
    private final UUID hashValue; // 文件内容的UUID由于更容易传输
    ...
    public static ServerPlacement fromJson(final JsonObject obj, final SyncmaticaContext context) {
        if (obj.has("id")
                && obj.has("file_name")
                && obj.has("hash")
                && obj.has("origin")
            ...
            if (obj.has("lastModifiedBy")) {
                newPlacement.lastModifiedBy = context.playerIdentifierProvider
